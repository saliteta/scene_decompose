...
from gsplat_ext.stratgy.feature import FeatureStrategy
from scene_decompose.train.gsplat_viewer import GsplatViewer, GsplatRenderTabState
...
class Config:
    ...
    save_steps: List[int] = field(default_factory=lambda: [6999])
    # Whether to save ply file (storage size can be large)
    save_ply: bool = False
    # Steps to save the model as ply
    ply_steps: List[int] = field(default_factory=lambda: [6999])
    ...
    strategy: Union[DefaultStrategy, MCMCStrategy, FeatureStrategy] = field(
        default_factory=FeatureStrategy
    )
    ...
    def adjust_steps(self, factor: float):
        ...
        if isinstance(strategy, DefaultStrategy):
            ...
            strategy.refine_stop_iter = int(strategy.refine_stop_iter * factor)
            strategy.refine_every = int(strategy.refine_every * factor)
        elif isinstance(strategy, FeatureStrategy):
            strategy.refine_every = int(strategy.refine_every * factor)
        else:
...
if __name__ == "__main__":
    ...
    configs = {
        ...
        "feature": (
            "Gaussian splatting training using no densification, clamp larger scale GS to normal.",
            Config(
