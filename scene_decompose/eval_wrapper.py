...
class Runner:
    ...
    def set_splat(self, distill_args: DistillArgs):
        if distill_args.method == "2DGS":
            ...
            self.splats.from_file(
                distill_args.ckpt, feature_path=distill_args.feature_ckpt
            )
            ...
            self.splats = HierachicalPrimitive(
                with_feature=True
            )  # Initialize with feature support
            ...
            if "feature" not in self.splats.source:
                raise ValueError(
                    "Loaded HierachicalPrimitive does not contain 'feature' key. Ensure the file was saved with features."
                )
            database = FeatureDatabase(
                num_layers=len(self.splats.source["feature"]),
                features=self.splats.source["feature"],
            )
            self.query_system = FeatureQuerySystem(database)
            self.feature_extractor = FeatureProcessor(
    ...
    def get_features(
        ...
        if self.distill_args.method != "H_GS":
            ...
            features = features.mean(
                dim=(0, 1)
            )  # (C,) - mean over spatial dimensions H and W
    ...
    def distill(self, distill_args: DistillArgs, data_args: DataArgs):
        ...
        for i, data in tqdm(
            ...
            if self.distill_args.method != "H_GS":
                ...
                query_attention_scores = self.splats.propagate_feature(
                    query_attention_scores.unsqueeze(-1)
                )[-1].squeeze(-1)
            ...
            gt_attention_scores = torch.full(
            ...
            similarity = self.distribution_similarity(
                query_attention_scores, gt_attention_scores
            )
            precision_at_10 = self.precision_at(
                query_attention_scores, gt_attention_scores, at=10
            )
            recall_at_10 = self.recall_at(
                query_attention_scores, gt_attention_scores, at=10
            )
            ...
            results.append(
                {
                    ...
                    "precision_at_10": (
                        precision_at_10.item()
                        if isinstance(precision_at_10, torch.Tensor)
                        else float(precision_at_10)
                    ),
                    "recall_at_10": (
                        recall_at_10.item()
                        if isinstance(recall_at_10, torch.Tensor)
                        else float(recall_at_10)
                    ),
                }
            )
        ...
        print(
            f"Average similarity: {sum(r['similarity'] for r in results) / len(results):.4f}"
        )
        print(
            f"Average precision at 10: {sum(r['precision_at_10'] for r in results) / len(results):.4f}"
        )
        print(
            f"Average recall at 10: {sum(r['recall_at_10'] for r in results) / len(results):.4f}"
        )

    def distribution_similarity(
        self, query_attention_scores: torch.Tensor, gt_attention_scores: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if query_max != query_min:
            query_normalized = (
                2.0 * (query_attention_scores - query_min) / (query_max - query_min)
                - 1.0
            )
        else:
        ...
        if gt_max != gt_min:
            gt_normalized = (
                2.0 * (gt_attention_scores - gt_min) / (gt_max - gt_min) - 1.0
            )
        else:
        ...
        if gt_length > 0:
            ...
            similarity = torch.tensor(
                0.0, device=similarity.device, dtype=similarity.dtype
            )
    ...
    def precision_at(
        self,
        query_attention_scores: torch.Tensor,
        gt_attention_scores: torch.Tensor,
        at=10,
    ) -> torch.Tensor:
        """
        ...
        _, top_indices = torch.topk(
            query_attention_scores,
            k=int(at / 100 * query_attention_scores.shape[0]),
            dim=0,
        )
        ...
        gt_attention_scores_binary = torch.where(
            gt_attention_scores < 0,
            torch.tensor(
                0.0, device=gt_attention_scores.device, dtype=gt_attention_scores.dtype
            ),
            torch.tensor(
                1.0, device=gt_attention_scores.device, dtype=gt_attention_scores.dtype
            ),
        )

        return (
            query_attention_scores * gt_attention_scores_binary
        ).sum() / query_attention_scores.sum()
    ...
    def recall_at(
        ...
        return (
            query_attention_scores * gt_attention_scores_binary
        ).sum() / gt_attention_scores_binary.sum()
