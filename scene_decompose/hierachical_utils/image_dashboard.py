...
class FeatureProcessor:
    ...
    def _setup_model(self):
        ...
        print(
            f"\033[92m[JAFAR] Model initialized successfully with backbone: {self.backbone} and model path: {self.model_path}\033[0m"
        )
        print(
            f"\033[92m[K-means] K-means initialized successfully with {self.k} clusters and {self.n_pca} PCA components\033[0m"
        )
...
def launch_image_dashboard(
    ...
    def _process(img: Image.Image, model_path: str = "", use_jafar: bool = True):
        ...
        if (
            ...
            and model_path
            and (
                feature_processor is None or feature_processor.model_path != model_path
            )
        ):
    ...
    with gr.Blocks(title="2D Image PCA Dashboard") as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Group():
                    ...
                    use_jafar = gr.Checkbox(
                        value=True, label="Use JAFAR Feature Extraction"
                    )
        ...
        btn.click(
            _process,
            inputs=[img_in, model_path, use_jafar],
            outputs=[out_img, out_txt, out_pca_img_quantized, out_pca_img_original],
        )
