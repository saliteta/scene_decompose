...
class GeneralViewerWrapper(GeneralViewer):
    def __init__(
        ...
        self.setup()
        super(GeneralViewer, self).__init__(
            server, self.render_function, None, "rendering"
        )
    ...
    def feature_pca(self) -> torch.Tensor:
        ...
        projected_features, pc_directions, mean = pca_torch_extract_directions(
    ...
    def attention_scores(
        self,
    ) -> torch.Tensor:  # [N,4] in cuda, first is score, last three are color
        ...
        if self._attention_scores is None:
            ...
            # Warn user
            print(
                "\033[33m[GeneralViewerWrapper] Warning: Attention scores not initialized. "
        ...
        if self._attention_scores.shape[1] == 1:
            ...
            self._attention_scores = torch.cat(
                [self._attention_scores, attention_colors], dim=1
            )

            print(
                "\033[92m[GeneralViewerWrapper] Attention scores converted to colors successfully!\033[0m"
            )
    ...
    def attention_score_to_color(self, attention_scores: torch.Tensor) -> torch.Tensor:
        ...
        print(
            f"\033[92m[GeneralViewerWrapper] Attention scores converted to colors successfully!, get {colors.shape} colors\033[0m"
        )
        return colors

    @property
    def gt_attention_scores(
        self,
    ) -> torch.Tensor:  # [N,4] in cuda, first is score, last three are color
        ...
        if self._gt_attention_scores.shape[1] == 1:
            ...
            self._gt_attention_scores = torch.cat(
                [self._gt_attention_scores, attention_colors], dim=1
            )
    ...
    def cached_renderer(self) -> Renderer:
        ...
        if self._cached_renderer is None:
            ...
            if self.render_tab_state.render_mode == "Attention":
                ...
                self._cached_renderer.primitives._feature = self.gt_attention_scores[
                    :, 1:
                ]
        ...
        if self.render_tab_state.render_mode == "RGB":
            ...
            if (
                ...
                == self.render_tab_state.current_render_mode
            ):
                return self._cached_renderer
            else:
                self.render_tab_state.current_render_mode = (
                ...
                if self.render_tab_state.render_mode == "Feature":
                    ...
                    self._cached_renderer.primitives._feature = self.attention_scores[
                        :, 1:
                    ]
                elif self.render_tab_state.render_mode == "gt_Attention":
                    self._cached_renderer.primitives._feature = (
                        self.gt_attention_scores[:, 1:]
                    )
                else:
                    raise ValueError(
                        f"Invalid render mode: {self.render_tab_state.render_mode}"
                    )
                return self._cached_renderer
    ...
    def gt_attention_query(
        self, gt_camera_pose: Dict[str, torch.Tensor]
    ) -> torch.Tensor:
        """
        ...
        splat_features_per_image, splat_weights_per_image, ids = (
            self.cached_renderer.inverse_render(
                K=K.to("cuda").unsqueeze(0),
                extrinsic=camera_pose.to("cuda").unsqueeze(0),
                width=width,
                height=height,
        ...
        attention_scores = torch.full(
            (N,), -1.0, device="cuda", dtype=splat_features_per_image.dtype
        )
        ...
        print(
            f"\033[33m[GeneralViewerWrapper] GT attention scores min: {attention_scores.min()}, GT attention scores max: {attention_scores.max()}\033[0m"
        )
        return attention_scores
    ...
    def _on_dashboard_submit(self, query_data: QueryData):
        ...
        if query_data.query_type == "image_feature":
            ...
            self._attention_scores = self.attention_query(query_feature).unsqueeze(
                -1
            )  # [N, 1]
            print(
                f"\033[92m[GeneralViewerWrapper] Querying image feature successfully!, get {self._attention_scores.shape} attention scores\033[0m"
            )
        elif query_data.query_type == "gt_camera_pose":
            gt_camera = query_data.gt_camera
            self._gt_attention_scores = self.gt_attention_query(gt_camera).unsqueeze(
                -1
            )  # [N, 1]
            print(
                f"\033[92m[GeneralViewerWrapper] Querying gt camera pose successfully!, get {self._gt_attention_scores.shape} attention scores\033[0m"
            )
        else:
    ...
    def _populate_rendering_tab(self):
        ...
        with self._rendering_folder:
            ...
            def _(_evt):
                ...
                print(
                    f"\033[92m[GeneralViewerWrapper] GT database path set to {self._gt_db}\033[0m"
                )
    ...
    @torch.no_grad()
    def render_function(
        self, camera_state: CameraState, render_tab_state: GeneralViewerWrapperState
    ):
        ...
        if state.render_mode == "RGB":
            image = self.cached_renderer.render(
