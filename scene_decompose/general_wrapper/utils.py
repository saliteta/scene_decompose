...
def pca_quantized_feature(
    features_flat: torch.Tensor, k: int, random_state: int = 42, n_pca: int = 16
) -> torch.Tensor:
    ...
    cluster_centers_pca = (
        torch.from_numpy(kmeans.cluster_centers_).float().to(features_pca_flat.device)
    )
