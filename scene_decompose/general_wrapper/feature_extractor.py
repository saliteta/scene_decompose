...
class FeatureExtractor(ABC):
    ...
    ]  # Takes transformed tensor, returns (B, H, W, C)
    transform: Callable[
        [torch.Tensor], torch.Tensor
    ]  # Takes (H, W, 3) [0-255], returns model input
    ...
    def extract_features(self, image: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            image.dim() == 3 and image.shape[2] == 3
    ...
    def setup_transform(self) -> None:
        ...
        if hasattr(self._openclip_transform, "transforms"):
            for transform in self._openclip_transform.transforms:
                ...
                if (
                    "_convert_to_rgb" in transform_name
                    or "_convert_to_rgb" in transform_repr
                    or "convert" in transform_name.lower()
                    and "rgb" in transform_name.lower()
                ):
                    continue

                # Skip ToTensor() (converts PIL Image to tensor, we already have a tensor)
                if (
                    isinstance(transform, T.ToTensor)
                    or transform_type_name == "ToTensor"
                ):
                    continue
...
class FeatureExtractorGT:
    ...
    def setup_model(self) -> None:
        parser = Parser(
            self.config.model_path, test_every=10000000
        )  # use all data for query
...
if __name__ == "__main__":
    config = FeatureExtractorConfig(
        model_type="gt", model_path="/data1/COLMAP_RESULT/CUHK_LOWER", device="cuda"
    )
