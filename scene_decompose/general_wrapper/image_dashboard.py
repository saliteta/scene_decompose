...
try:
    ...
    from scene_decompose.hierachical_utils.pca_utils import apply_pca_directions_torch, pca_torch_extract_directions
    from scene_decompose.general_wrapper.query_data import QueryData
except ImportError:
    # Fall back to relative imports (when imported as a module)
    from ..hierachical_utils.pca_utils import apply_pca_directions_torch, pca_torch_extract_directions
...
def extract_image_feature_token(
    ...
    # Extract features
    features = feature_processor.extract_features(tensor_img)  # [B, H', W', C] or [H', W', C]
...
class FeatureProcessor:
    ...
    def __init__(self, 
        ...
        print("="*60)
        print(f"\033[92m[FeatureProcessor] {self.model_type.upper()} model initialized successfully\033[0m")
        if self.model_type == "jafar":
            print(f"\033[92m[FeatureProcessor] Backbone: {self.backbone}, Model path: {self.model_path}\033[0m")
        elif self.model_type == "openclip":
            print(f"\033[92m[FeatureProcessor] Architecture: {self.backbone}\033[0m")
        print(f"\033[92m[K-means] K-means initialized with {self.k} clusters and {self.n_pca} PCA components\033[0m")
        print("="*60)
    ...
    def _setup_extractor(self):
        ...
        if self.model_type == "jafar":
            ...
            raise ValueError(f"Unsupported model_type: {self.model_type}. Must be 'jafar' or 'openclip'")
    ...
    def extract_features(self, image: torch.Tensor) -> torch.Tensor:
        ...
        print(f"\033[92m[FeatureProcessor] Extracted features shape: {features.shape}\033[0m")
...
class GradioUI:
    ...
    def setup_gt_db(self):
        ...
        self.gt_db = dataset
        print(f"\033[92m[GradioUI] GT DB setup successfully with {len(self.gt_db)} images\033[0m")
        print(f"\033[92m[GradioUI] Available GT images: {len(self.gt_names)} names\033[0m")

    def ensure_feature_processor(self, model_path: str, model_type: str = "jafar", backbone: str = "vit_large_patch16_dinov3.lvd1689m") -> None:
        """
        ...
        needs_reinit = (
            self.feature_processor is None or
            self.feature_processor.model_type != model_type.lower() or
            self.feature_processor.model_path != model_path or
            self.feature_processor.backbone != backbone
        )
        
        if needs_reinit:
            print(f"[GradioUI] Initializing FeatureProcessor: {model_type.upper()} with {backbone} @ {model_path}")
    ...
    def process_image_query(self,
                            ...
                            model_path: str,
                            model_type: str = "jafar",
                            backbone: str = "vit_large_patch16_dinov3.lvd1689m") -> Tuple[Optional[Image.Image], str, Optional[Image.Image], Optional[Image.Image]]:
        """
        ...
        if img is None:
            return None, "No image provided. Please upload an image or provide a local path.", None, None
        ...
        self.ensure_feature_processor(model_path, model_type=model_type, backbone=backbone)
        if self.feature_processor is None:
            return img, f"{model_type.upper()} feature extraction not available. Please provide model path.", None, None
        ...
        if self.pca_directions is not None and self.pca_mean is not None:
            try:
                cluster_centers, pca_img_quantized, pca_img_original, pca_info = process_quantized_jafar_pca(
                    img, self.feature_processor, self.pca_directions, self.pca_mean, shape
                )
            except Exception as e:
        ...
        if self.on_submit is not None:
            q = QueryData(query_type="image_feature", image_feature_token=feature_token, gt_camera=None)
            self.on_submit(q)
    ...
    def process_gt_selection(self, gt_index: int) -> Tuple[Dict[str, torch.Tensor], str]:
        """
        ...
        if gt_index < 0 or gt_index >= len(self.gt_names):
            return None, f"Invalid GT index: {gt_index}. Must be between 0 and {len(self.gt_names)-1}."
...
def launch_image_dashboard(
    ...
    ui = GradioUI(on_submit=on_submit, pca_directions=pca_directions, pca_mean=pca_mean, gt_db=gt_db, feature_processor=feature_processor)
    ...
    gt_dropdown_choices = [f"{i}: {name}" for i, name in enumerate(gt_options)] if gt_options else []
    
    # =========================================================================
    # Image Query Processing Function
    # =========================================================================
    def _process_image_query(img: Optional[Image.Image], local_path: str, model_path: str, model_type: str, backbone: str):
        return ui.process_image_query(img, local_path, model_path, model_type=model_type, backbone=backbone)
    
    # =========================================================================
    # Gradio Interface
    # =========================================================================
    with gr.Blocks(title="Image Query & Ground Truth Dashboard") as demo:
        gr.Markdown("## Image Query & Ground Truth Selection Dashboard\n"
        ...
        btn_image_query.click(
            ...
            outputs=[out_img_query, out_txt_query, out_pca_img_quantized, out_pca_img_original]
        )
