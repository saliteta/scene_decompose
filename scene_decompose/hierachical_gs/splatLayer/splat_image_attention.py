...
class SplatImageAttention(nn.Module):
    ...
    def forward(
        self, splat_block_features: torch.Tensor, image_features: torch.Tensor
    ) -> torch.Tensor:
        # Input would be [2**layer_level, 2**token_level, C] and [K, C]
        assert (
            splat_block_features.ndim == 3
        ), f"Splat block features must be a 3D tensor, got {splat_block_features.ndim}"
        assert (
            image_features.ndim == 2
        ), f"Image features must be a 2D tensor, got {image_features.ndim}"
        assert (
            splat_block_features.shape[1] == image_features.shape[0]
        ), f"Token per block should be same as token per image,\
             got token per block: {splat_block_features.shape[1]} and token per image: {image_features.shape[0]}"
        assert (
            splat_block_features.shape[2] == image_features.shape[1]
