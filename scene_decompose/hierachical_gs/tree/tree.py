...
class HierarchicalTree:
    ...
    def _build_tree(self):
        ...
        print(
            f"Building hierarchical tree with depth {self.max_depth}, {self.num_points} points"
        )
        ...
        for layer_id in range(self.max_depth + 1):
            ...
            for local_id in range(num_nodes):
                ...
                if layer_id < self.max_depth:  # Not root
                    ...
                    parent_id = self._compute_global_node_id(
                        layer_id + 1, parent_local_id
                    )

                if layer_id > 0:  # Not leaf
                    left_child_id = self._compute_global_node_id(
                        layer_id - 1, local_id * 2
                    )
                    right_child_id = self._compute_global_node_id(
                        layer_id - 1, local_id * 2 + 1
                    )
    ...
    def _assign_points_to_leaves(self):
        ...
        for i, node in enumerate(leaf_layer.nodes):
            ...
            if start_idx < self.num_points:
                node.point_indices = torch.arange(
                    start_idx, end_idx, device=self.points.device
                )
    ...
    def _compute_node_data(self):
        ...
        for i, layer in tqdm(
            ...
            desc="Computing node data, time decreasing exponentially",
    ...
    def _fill_empty_node_from_sibling(self, empty_node: TreeNode, layer: LayerInfo):
        ...
        if sibling_node is not None:
            ...
            if sibling_node.features is not None:
                ...
                empty_node.location = torch.zeros(
                    3, device=self.points.device, dtype=self.points.dtype
                )
                if self.features is not None:
                    empty_node.features = torch.zeros(
                        self.feature_dim,
    ...
    def _find_sibling_with_data(
        self, empty_node: TreeNode, layer: LayerInfo
    ) -> Optional[TreeNode]:
        """
        ...
        for node in layer.nodes:
            if (
                node != empty_node
                and node.point_indices is not None
                and node.point_indices.numel() > 0
            ):
    ...
    def _find_actual_sibling_with_data(
        self, empty_node: TreeNode
    ) -> Optional[TreeNode]:
        """
        ...
        for sibling_id in parent_node.children_ids:
            if sibling_id != empty_node.node_id:
                ...
                if (
                    sibling_node.point_indices is not None
                    and sibling_node.point_indices.numel() > 0
                ):
                    return sibling_node

        # If no direct sibling has data, look for any node with data
        return self._find_sibling_with_data(
            empty_node, self.layers[empty_node.layer_id]
        )
        ...
        for node in layer.nodes:
            if node.features is not None:
                ...
                zero_features = torch.zeros(
                    self.feature_dim, device=self.points.device, dtype=self.points.dtype
                )
    ...
    def get_parent_child_features(self, parent_layer_id: int) -> torch.Tensor:
        ...
        for parent_node in parent_layer.nodes:
            ...
            if child_features:
                parent_child_features = torch.stack(
                    child_features, dim=0
                )  # [num_children, feature_dim]
                grouped_features.append(parent_child_features)

        if grouped_features:
            return torch.stack(
                grouped_features, dim=0
            )  # [num_parents, num_children, feature_dim]
        else:
            return torch.zeros(
                0,
                0,
    ...
    def get_ancestor_descendant_features(
        self, source_layer_id: int, hops: int
    ) -> torch.Tensor:
        """
        ...
        for ancestor_local_id, ancestor_node in enumerate(ancestor_layer.nodes):
            ...
            if descendant_features:
                ancestor_descendant_features = torch.stack(
                    descendant_features, dim=0
                )  # [2^hops, feature_dim]
        ...
        if grouped_features:
            return torch.stack(
                grouped_features, dim=0
            )  # [num_ancestors, 2^hops, feature_dim]
        else:
            return torch.zeros(
                0,
                2**hops,
    ...
    def update_layer_features(self, layer_id: int, new_features: torch.Tensor):
        ...
        if new_features.shape[0] != len(layer.nodes):
            raise ValueError(
                f"Expected {len(layer.nodes)} features, got {new_features.shape[0]}"
            )

        for i, node in enumerate(layer.nodes):
            node.features = new_features[i]

    def reset_to_base_features(
        self,
        ...
        if new_features is not None:
            if new_features.shape[0] != self.num_points:
                raise ValueError(
                    f"Expected {self.num_points} features, got {new_features.shape[0]}"
                )
            if new_features.shape[1] != self.feature_dim:
                raise ValueError(
                    f"Expected {self.feature_dim} feature dimensions, got {new_features.shape[1]}"
                )
    ...
    def get_tree_info(self) -> Dict[str, Any]:
        ...
        for layer in self.layers:
            layer_info = {
                ...
                "has_features": (
                    layer.nodes[0].features is not None if layer.nodes else False
                ),
            }
        ...
        for layer in self.layers:
            for node in layer.nodes:
                if node.point_indices is not None:
                    node.point_indices = node.point_indices.to(
                        device, non_blocking=moving_to_cuda
                    )
                if node.location is not None:
                    node.location = node.location.to(
                        device, non_blocking=moving_to_cuda
                    )
                if node.features is not None:
                    node.features = node.features.to(
                        device, non_blocking=moving_to_cuda
                    )
