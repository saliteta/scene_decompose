...
class PipelineConfig:
    ...
    def from_yaml(cls, yaml_path: Union[str, Path]) -> "PipelineConfig":
        ...
        hierarchical_gs_config = HierarchicalGSConfig(
            **config_dict.get("hierachical_gs", {})
        )
...
class runner:
    ...
    def train(self):
        ...
        scene_names = [
            d
            for d in os.listdir(self.config.dataset_path)
    ...
    def lifting(self):
        ...
        for scene_name in scene_names:
            ...
            ckpt_path = (
                output_dir / "ckpts" / f"ckpt_{self.config.train.steps-1}_rank0.pt"
            )
            # Output feature path
            feature_path = (
                output_dir
    ...
    def hierachical_gs(self):
        ...
        for scene_name in scene_names:
            ...
            cmd = [
                ...
                str(h_gs_path),
                "-asl",
                str(self.config.hierachical_gs.attention_start_layer),
                "-hfa",
                str(self.config.hierachical_gs.hops_for_attention),
                "-v",
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pipeline configuration parser and runner"
    )
    parser.add_argument(
        "--config", type=str, default="eval.yml", help="Path to YAML configuration file"
    )
    parser.add_argument(
        "--list-scenes", action="store_true", help="List all scenes in the dataset"
    )
    parser.add_argument(
        "--run",
        action="store_true",
