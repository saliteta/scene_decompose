...
try:
    ...
    from scene_decompose.hierachical_gs.splatLayer.splat_image_attention import (
        SplatImageAttention,
    )
except ImportError:
...
class QuerySystem:
    ...
    def verbose(self) -> str:
        return (
            f"Query System: {self.database.name}\n"
    ...
    @abstractmethod
    def query(
        self, query: Union[str, torch.Tensor, None]
...
class LayerQuerySystem(QuerySystem):
    ...
    def query(
        self, query_text: Union[str, None] = None
    ) -> Union[List[torch.Tensor], None]:
        """
        ...
        if query_text is not None:
            assert isinstance(
                query_text, str
            ), f"Query text must be a string or None, but got {type(query_text)}"
            assert all(
                c in "01" for c in query_text
            ), f"Query text must only contain '0' and '1', but got {query_text}"
            assert (
                len(query_text) < self.num_layers
            ), f"Query text length must be less than the number of layers, but got {len(query_text)}"
        else:
            return None
        ...
        for layer in range(highest_layer_number, self.num_layers):
            index_range.append(
                torch.tensor(
                    [
                        layer_index << (layer - highest_layer_number),
                        ((layer_index + 1) << (layer - highest_layer_number)) - 1,
                    ]
                )
            )
        return index_range
...
class FeatureQuerySystem(LayerQuerySystem):
    ...
    def query(
        self,
        query_image: Union[torch.Tensor, str, None] = None,
        layer_level: int = 5,
        token_level: int = 5,
    ) -> Union[List[torch.Tensor], None]:
        """
        ...
        if query_image is None or isinstance(query_image, str):
            print(
                f"\033[92m[FeatureQuerySystem] Query image is None or str, running layer query system\033[0m"
            )
            return super().query(query_image)
        K, C = query_image.shape
        assert (
            layer_level + token_level <= self.num_layers
        ), f"Layer level and token level must be less than the number of layers, but got {layer_level} + {token_level} > {self.num_layers}"
        assert (
            K == 2**token_level
        ), f"Query image must have K tokens, but got {K}, Query level is {token_level}"
        assert (
            C == self.database.features[layer_level].shape[1]
        ...
        splat_block_features: torch.Tensor = self._prepare_splat_block_features(
            layer_level, token_level
        )
        attn_scores: torch.Tensor = self.splat_image_attention(
            splat_block_features, query_image
        )
        assert attn_scores.shape == (
            2**layer_level,
        ), f"Attention scores must have 2**layer_level elements, but got {attn_scores.shape}"
        return attn_scores

    def _prepare_splat_block_features(
        self, layer_level: int, token_level: int
    ) -> torch.Tensor:
        """
        ...
        flat_features: torch.Tensor = self.database.features[
            -(layer_level + token_level + 1)
        ]  # [N, C]
        block_features: torch.Tensor = flat_features.view(
            block_number, token_number, -1
        )  # [2**layer_level, 2**token_level, C]
